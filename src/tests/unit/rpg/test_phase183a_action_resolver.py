...
_ar = _load("action_resolver", "app/rpg/action_resolver.py")
...
class TestResolveAttackRoll:
    def test_hit_with_seed(self):
        ...
        result = _ar.resolve_attack_roll(attacker, defender, weapon, seed=42)
    ...
    def test_miss_possible(self):
        ...
        miss_count = sum(1 for s in range(100) if _ar.resolve_attack_roll(attacker, defender, weapon, seed=s)["outcome"] == "miss")
        assert miss_count > 0
    ...
    def test_crit_possible(self):
        ...
        crit_count = sum(1 for s in range(100) if _ar.resolve_attack_roll(attacker, defender, weapon, seed=s)["is_crit"])
        assert crit_count > 0
    ...
    def test_weapon_damage_affects_result(self):
        ...
        r1 = _ar.resolve_attack_roll(attacker, defender, weak, seed=10)
        r2 = _ar.resolve_attack_roll(attacker, defender, strong, seed=10)
    ...
    def test_strength_affects_melee(self):
        ...
        r1 = _ar.resolve_attack_roll(weak, defender, weapon, seed=5)
        r2 = _ar.resolve_attack_roll(strong, defender, weapon, seed=5)
        if r1["outcome"] in ("hit", "crit") and r2["outcome"] in ("hit", "crit"):
            assert r2["damage"] >= r1["damage"]
    ...
    def test_armor_reduces_damage(self):
        ...
        r1 = _ar.resolve_attack_roll(attacker, no_armor, weapon, seed=3)
        r2 = _ar.resolve_attack_roll(attacker, heavy_armor, weapon, seed=3)
...
class TestResolveNoncombatCheck:
    def test_success(self):
        ...
        result = _ar.resolve_noncombat_check(ps, "persuade", "easy", seed=42)
    ...
    def test_difficulty_affects_outcome(self):
        ...
        easy_success = sum(1 for s in range(50) if _ar.resolve_noncombat_check(ps, "sneak", "easy", seed=s)["outcome"] in ("success", "critical_success"))
        hard_success = sum(1 for s in range(50) if _ar.resolve_noncombat_check(ps, "sneak", "very_hard", seed=s)["outcome"] in ("success", "critical_success"))
        assert easy_success >= hard_success

    def test_hack_uses_intelligence(self):
        ps = {"stats": {"intelligence": 16}, "skills": {"hacking": {"level": 3}}}
        result = _ar.resolve_noncombat_check(ps, "hack", "normal", seed=1)
...
class TestResolvePlayerAction:
    ...
    def test_item_action(self):
        ...
        result = _ar.resolve_player_action(sim, action, seed=1)
...
class TestComputeDefenseRating:
    def test_no_armor(self):
        ...
        assert _ar.compute_defense_rating(actor) == 0

    def test_with_armor(self):
        actor = {"stats": {"constitution": 14}, "inventory_state": {"equipment": {"chest": {"combat_stats": {"defense_bonus": 5}}}}}
        rating = _ar.compute_defense_rating(actor)
        assert rating >= 5

    def test_high_con_adds_defense(self):
        low = _ar.compute_defense_rating({"stats": {"constitution": 8}})
        high = _ar.compute_defense_rating({"stats": {"constitution": 18}})
        assert high > low


class TestSelectEquippedWeapon:
    def test_unarmed_fallback(self):
        w = _ar.select_equipped_weapon({})
    ...
    def test_equipped_weapon(self):
        ...
        w = _ar.select_equipped_weapon(ps)
