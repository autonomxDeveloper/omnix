 ...
 *
 * Replaces the legacy showAdventureSetup() / buildSetupModal() with a proper
 * multi-step adventure builder backed by AdventureSetup, validated by the
 * backend, previewed in real-time, and launched through the structured creator
 * pipeline.
 *
 * Steps:
 *   1. Template / Basics  (genre, setting, premise, title, player name, mood, difficulty)
 *   2. Rules / Tone       (hard rules, soft tone, forbidden content, canon notes)
 *   3. World Seeds        (factions, locations, NPCs — repeatable editors)
 *   4. Start State        (starting location, starting NPCs, pacing, content balance)
 *   5. Review / Preview   (validation, counts, preview summary, launch)
 *
 * Exposes:
 *   AdventureBuilder.open()   — show the wizard
 *   AdventureBuilder.close()  — hide the wizard
 */

/* global AdventureBuilderApi */
...
var AdventureBuilder = (function () {
    ...
    var DRAFT_KEY = 'omnix_rpg_adventure_builder_v1';
    var STEP_COUNT = 5;
    var DEBOUNCE_MS = 400;
    ...
    var setup = _loadDraft() || _defaultSetup();
    ...
    function _saveDraft() {
        try { localStorage.setItem(DRAFT_KEY, JSON.stringify(setup)); } catch (e) { /* ignore */ }
    }
    ...
    function _runValidation() {
        ...
        _saveDraft();
        AdventureBuilderApi.validateSetup(setup).then(function (res) {
            validationResult = res.validation || null;
            _renderInlineIssues();
            _updateLaunchButton();
    ...
    function _runPreview() {
        AdventureBuilderApi.previewSetup(setup).then(function (res) {
            previewResult = res;
    ...
    function _renderFooter() {
        ...
        if (saveBtn) saveBtn.addEventListener('click', function () {
            _readCurrentStepIntoSetup(); _saveDraft();
    ...
    function _updateLaunchButton() {
        ...
        if (!btn) return;
        btn.disabled = !!(validationResult && validationResult.blocking);
    }
    ...
    function _renderTemplateGallery() {
        ...
        gal.querySelectorAll('.ab-tpl-card').forEach(function (card) {
            card.addEventListener('click', function () {
                ...
                if (!name) { setup = _defaultSetup(); _renderStep(); return; }
                _applyTemplate(name);
            });
        });
    }
    ...
    function _applyTemplate(name) {
        AdventureBuilderApi.buildTemplate(name).then(function (res) {
            if (res.success && res.setup) {
                var tplData = res.setup;
                // Merge template into setup — keep any user-set metadata
                Object.keys(tplData).forEach(function (k) {
                    if (tplData[k] !== null && tplData[k] !== undefined && tplData[k] !== '') {
                        setup[k] = tplData[k];
                    }
                });
    ...
    function _attachEntityDelete(container, type) {
        container.querySelectorAll('.ab-entity-del').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (type === 'faction') { setup.factions.splice(idx, 1); _renderFactionCards(); }
                else if (type === 'location') { setup.locations.splice(idx, 1); _renderLocationCards(); }
                else if (type === 'npc') { setup.npc_seeds.splice(idx, 1); _renderNpcCards(); }
                _saveDraft();
            });
        });
    }
    ...
    function _renderReviewValidation() {
        ...
        if (!el) return;
        if (!validationResult || !validationResult.issues || !validationResult.issues.length) {
    ...
    function _renderPreviewPanel() {
        ...
        if (!el) return;
        if (!previewResult) { el.innerHTML = '<p class="ab-muted">No preview available.</p>'; return; }
        ...
        if (previewResult.ok === false) {
            ...
            var ctx = previewResult.resolved_context || {};
        ...
        // Also show validation from preview result
        var pVal = previewResult.validation;
    ...
    function _renderInlineIssues() {
        if (!overlayEl || !validationResult) return;
        // Clear old
    ...
    // ─── Chip editor (repeatable text rows) ────────────────────────────────────
