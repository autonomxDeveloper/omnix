...
(function () {
    ...
    function collectGlobalImageSettingsForm() {
        return {
            enabled: !!(el('imgEnabled') && el('imgEnabled').checked),
            auto_unload_on_disable: !!(el('imgAutoUnload') && el('imgAutoUnload').checked),
            provider: (el('imgProvider') && el('imgProvider').value) || 'flux_klein',
            chat: {
                auto_generate_images: !!(el('imgChatAuto') && el('imgChatAuto').checked),
                style: (el('imgChatStyle') && el('imgChatStyle').value) || '',
            },
            story: {
                auto_generate_scene_images: !!(el('imgStorySceneAuto') && el('imgStorySceneAuto').checked),
                auto_generate_cover_images: !!(el('imgStoryCoverAuto') && el('imgStoryCoverAuto').checked),
                style: (el('imgStoryStyle') && el('imgStoryStyle').value) || 'story',
            },
            flux_klein: {
                download_dir: (el('imgFluxDownloadDir') && el('imgFluxDownloadDir').value) || 'image',
                local_dir: (el('imgFluxLocalDir') && el('imgFluxLocalDir').value) || '',
                scene_width: Number((el('imgSceneWidth') && el('imgSceneWidth').value) || 1344),
                scene_height: Number((el('imgSceneHeight') && el('imgSceneHeight').value) || 768),
                portrait_width: Number((el('imgPortraitWidth') && el('imgPortraitWidth').value) || 768),
                portrait_height: Number((el('imgPortraitHeight') && el('imgPortraitHeight').value) || 1024),
            }
        };
    }

    function bindGlobalImageSettingsEvents(container) {
        var runtimeBox = el('imgRuntimeStatusBox');
        var saveBtn = el('imgSettingsSaveBtn');
        var downloadBtn = el('imgSettingsDownloadBtn');
        var loadBtn = el('imgSettingsLoadBtn');
        var unloadBtn = el('imgSettingsUnloadBtn');
        var refreshBtn = el('imgSettingsRefreshRuntimeBtn');
        var jobsBtn = el('imgSettingsListJobsBtn');
        var manifestBtn = el('imgSettingsManifestBtn');
        var cleanupBtn = el('imgSettingsCleanupAssetsBtn');
        ...
        if (loadBtn) {
            loadBtn.addEventListener('click', function() {
                fetch('/api/image/provider/load', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ provider: (el('imgProvider') && el('imgProvider').value) || 'flux_klein' })
                }).then(function(r) { return r.json(); }).then(function(data) {
                    if (runtimeBox) runtimeBox.textContent = JSON.stringify(data, null, 2);
                    refreshRuntime();
                });
    ...
    function loadGlobalImageSettings(container) {
        ...
        loadVisualSettings();
        loadGlobalImageSettings(globalImageContainer);
